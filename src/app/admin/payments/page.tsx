...
import { fetchPayments, getReceiptLink, generateReceipt, type AdminPayment, type PaymentStatus } from "@/store/paymentsAdminSlice";
...
export default function AdminPaymentsPage() {
  ...
  const { items: payments, status, error } = useAppSelector((s) => s.paymentsAdmin);
  ...
  useEffect(() => {
    if (token && status === "idle") {
      dispatch(fetchPayments()).unwrap().catch(() => {});
    }
  ...
  const filtered = useMemo(() => {
    ...
    return payments.filter((p) => {
      ...
      const matchesProgramType = !programTypeFilter || (p.fee?.program?.program_type === programTypeFilter);
      const matchesLevel = !levelFilter || (p.level === levelFilter);
      const d = new Date(p.payment_date);
      const matchesStart = !start || d >= start;
      const matchesEnd = !end || d <= end;
      return matchesQuery && matchesStatus && matchesProgramType && matchesLevel && matchesStart && matchesEnd;
    });
  ...
  const onGetReceipt = async (id: number) => {
    try {
      ...
      const msg = typeof err === "string" ? err : err instanceof Error ? err.message : "Failed to get receipt";
      toast.error(msg);
    }
  };
  ...
  const onGenerateReceipt = async (id: number) => {
    try {
      ...
      const msg = typeof err === "string" ? err : err instanceof Error ? err.message : "Failed to generate receipt";
      toast.error(msg);
    }
  };
  ...
  const onExportExcel = async () => {
    try {
      ...
      // Filters summary
      const filterText = `Status: ${statusFilter || "All"} | Program Type: ${programTypeFilter || "All"} | Level: ${levelFilter || "All"} | Date: ${startDate || "Any"} - ${endDate || "Any"}`;
  ...
  return (
    <div className="space-y-6">
      {/* Header Section */}
      <div className="bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-600 rounded-2xl p-4 sm:p-6 text-white">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold mb-2">Payment Management</h1>
            <p className="text-blue-100 text-sm sm:text-base">Monitor and manage all student payments</p>
          </div>
          <button 
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-4 sm:p-6 border border-gray-200 dark:border-gray-700 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">{paymentStats.total.toLocaleString()}</p>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-4 sm:p-6 border border-gray-200 dark:border-gray-700 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl sm:text-2xl font-bold text-emerald-600">{paymentStats.successful.toLocaleString()}</p>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-4 sm:p-6 border border-gray-200 dark:border-gray-700 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl sm:text-2xl font-bold text-amber-600">{paymentStats.pending.toLocaleString()}</p>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-4 sm:p-6 border border-gray-200 dark:border-gray-700 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl sm:text-2xl font-bold text-red-600">{paymentStats.failed.toLocaleString()}</p>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-4 sm:p-6 border border-gray-200 dark:border-gray-700 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl sm:text-2xl font-bold text-green-600">₦{paymentStats.totalRevenue.toLocaleString()}</p>
            </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl p-4 sm:p-6 border border-gray-200 dark:border-gray-700 shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3 sm:gap-4">
          <div className="space-y-2">
            <label className="text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300">Search</label>
          ...
          <div className="space-y-2">
            <label className="text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300">Program Type</label>
            <select
          ...
          <div className="space-y-2">
            <label className="text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300">Level</label>
            <select
              value={levelFilter}
          ...
          <div className="space-y-2">
            <label className="text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300">Start Date</label>
            <div className="relative">
          ...
          <div className="space-y-2">
            <label className="text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300">End Date</label>
            <div className="relative">
              <CalendarDaysIcon className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400" />
              <input 
                type="date" 
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
         ...
         <div className="hidden lg:block overflow-x-auto">
           <table className="w-full">
             <thead className="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
               <tr>
                 <th className="text-left px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white">Date</th>
                 <th className="text-left px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white">Student</th>
                 <th className="text-left px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white">Email</th>
                 <th className="text-left px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white">Amount</th>
                 <th className="text-left px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white">Status</th>
                 <th className="text-left px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white">Fee</th>
                 <th className="text-left px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white">Department</th>
                 <th className="text-left px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white">Program</th>
                 <th className="text-left px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white">Level</th>
                 <th className="text-left px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white">Reference</th>
                 <th className="text-left px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white">Receipt</th>
             ...
             <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                 ...
                 <tr>
                   <td colSpan={12} className="px-6 py-12 text-center">
                     <div className="flex flex-col items-center space-y-2">
                       ...
                       <p className="text-red-600 font-medium">{error ?? "Failed to load payments"}</p>
                     </div>
                   </td>
                 </tr>
               )}
               ...
               {filtered.map((p) => (
                 <tr key={p.payment_id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                   ...
                   <td className="px-6 py-4">
                     <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium border ${getProgramTypeColor(p.fee?.program?.program_type ?? '')}`}>
         ...
         <div className="lg:hidden">
           ...
           <div className="divide-y divide-gray-200 dark:divide-gray-700">
             {filtered.map((p) => (
               <div key={p.payment_id} className="p-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                 <div className="flex items-start justify-between mb-3">
                   ...
                   <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium border ${getStatusColor(p.status)} flex-shrink-0 ml-2`}>
                     {getStatusIcon(p.status)}
                     {p.status}
                   </span>
                 </div>
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">Transaction information and receipt</p>
                </div>
              </div>
              <button
                onClick={() => { setDetailsOpen(false); setSelectedPayment(null); }}
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">JAMB:</span>
                    <span className="ml-2 font-medium text-gray-900 dark:text-white">
                      {selectedPayment.jamb_number ?? '-'}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">Matric:</span>
                    <span className="ml-2 font-medium text-gray-900 dark:text-white">
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <span className={`ml-2 inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium border ${getStatusColor(selectedPayment.status)}`}>
              ...
              <div className="bg-purple-50 dark:bg-purple-900/20 rounded-xl p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <span className={`ml-2 inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium border ${getProgramTypeColor(selectedPayment.fee?.program?.program_type ?? '')}`}>
                      <AcademicCapIcon className="w-3 h-3" />
                  ...
                  <div>
                    ...
                    <span className="ml-2 font-medium text-gray-900 dark:text-white">
                      {selectedPayment.level ? String(selectedPayment.level).replace('L', '') : '-'}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">Fee ID:</span>
                    <span className="ml-2 font-medium text-gray-900 dark:text-white">
              ...
              <div className="bg-green-50 dark:bg-green-900/20 rounded-xl p-4">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">Receipt Status:</span>
                    <span className="ml-2 font-medium text-gray-900 dark:text-white">
                      {selectedPayment.receipt_drive_url ? "Available" : "Not Generated"}
                    </span>
                  </div>
                  {selectedPayment.receipt_drive_url ? (
                    ...
                    <span className="text-gray-400 text-sm">No receipt available</span>
                  )}
                </div>
              </div>
            </div>
